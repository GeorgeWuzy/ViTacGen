...
class ObjectPushEnv(BaseObjectEnv):
    def __init__(
        ...
        if self.t_s_name == 'tactip':
            ...
            self.t_s_dynamics = {'stiffness': 100, 'damping': 200, 'friction': 10.0}
            
        # distance from goal to cause termination
        self.termination_pos_dist = 0.04 # Increase this dist if training cannot converge
        ...
        if self.arm_type in ['mg400', 'magician']:
            ...
            # this well_designed_pos is used for the object and the workframe.
            self.well_designed_pos = np.array([0.5, -0.15, self.obj_height/2]) 
        # work frame origin
        self.workframe_pos = np.array([0.5, -0.15, 0.01])
    ...
    def reset_domain_randomization(self): 
        # Plane
        plane_rgb = [0, 0, 0]
        plane_delta = random.uniform(0, 0.25) # TODO
        self._pb.changeVisualShape(  
            self.plane_id,   
            -1,   
            rgbaColor=[  
                min(1.0, max(0.0, plane_rgb[0] + plane_delta)),  
                min(1.0, max(0.0, plane_rgb[1] + plane_delta)),  
                min(1.0, max(0.0, plane_rgb[2] + plane_delta)),  
                1  
            ])  
        self._pb.changeVisualShape(  
            self.plane_id,   
        ...
        # Table
        delta_bottom = random.uniform(-20 / 255, 20 / 255)
        delta_top = random.uniform(-20 / 255, 20 / 255)
        self._pb.changeVisualShape(  
            self.table_id,  
            -1,  # base_plane   
            rgbaColor=[  
                min(1.0, max(0.0, 230 / 255 + delta_bottom)),
                min(1.0, max(0.0, 230 / 255 + delta_bottom)),
                min(1.0, max(0.0, 230 / 255 + delta_bottom)),
                1 
            ])  
        self._pb.changeVisualShape(  
            ...
            0,  # top_plate  
            rgbaColor=[
                min(1.0, max(0.0, 234 / 255 + delta_top)),
                min(1.0, max(0.0, 210 / 255 + delta_top)),
                min(1.0, max(0.0, 198 / 255 + delta_top)),
                1 
            ])  
        
        link_colors = {   # TODO
            "base_link": (227,235,237), 
            "shoulder_link": random.choice([(122, 127, 131), (132, 184, 223)]),
            "upper_arm_link": random.choice([(227,235,237), (122, 127, 131)]),
            "forearm_link": random.choice([(227,235,237), (122, 127, 131)]),
            "wrist_1_link": random.choice([(122, 127, 131), (132, 184, 223)]),
            "wrist_2_link": random.choice([(122, 127, 131), (132, 184, 223)]),
            "wrist_3_link": (227,235,237)
        }  

        link_colors = {key: tuple(value / 255. for value in color) for key, color in link_colors.items()} 

        for link_name in link_colors.keys():
            link_index = self.robot.arm.link_name_to_index[link_name]  
            delta = random.uniform(-20 / 255, 20 / 255)

            base_color = link_colors[link_name]  
            
            self._pb.changeVisualShape(  
                self.robot.robot_id,  
                link_index,  
                rgbaColor=[  
                    min(1.0, max(0.0, base_color[0] + delta)),
                    min(1.0, max(0.0, base_color[1] + delta)),
                    min(1.0, max(0.0, base_color[2] + delta)),
                    1
                ],  
    ...
    def setup_rgb_obs_camera_params(self): # According to your real env
        self.rgb_cam_pos = [0.212, 0.0, -0.02] # Camera focus: 1 - 0.455 * √3 
        self.rgb_cam_dist = 0.91 # + random.uniform(-0.05, 0.05)
        self.rgb_cam_yaw = 90 # + random.uniform(-10, 10)
        self.rgb_cam_pitch = -30 # + random.uniform(-5, 5)

        self.rgb_image_size = self._image_size
        # self.rgb_fov = 69
        self.rgb_fov = 42
        self.rgb_near_val = 0.2
        self.rgb_far_val = 10.0
    ...
    def setup_object(self):
        ...
        # Placeholder in initialization
        self.object_path = add_assets_path("rl_env_assets/nonprehensile_manipulation/object_push/pink_tea_box/model.urdf")
        # self.object_path = add_assets_path("rl_env_assets/nonprehensile_manipulation/object_push/mug/model.urdf")
        # self.object_path = add_assets_path("rl_env_assets/nonprehensile_manipulation/object_push/potted_meat_can/potted_meat_can.urdf")
        # self.object_path = add_assets_path("rl_env_assets/nonprehensile_manipulation/object_push/apple/apple.urdf")
        # self.object_path = add_assets_path("rl_env_assets/nonprehensile_manipulation/object_push/tomato_soup_can/tomato_soup_can.urdf")
    ...
    def reset_object(self):
        ...
        Now randomly selects from available object URDFs.
        """

        # Define available object paths
        object_paths = [ #
            "rl_env_assets/nonprehensile_manipulation/object_push/pink_tea_box/model.urdf", 
            "rl_env_assets/nonprehensile_manipulation/object_push/mug/mug.urdf",
            "rl_env_assets/nonprehensile_manipulation/object_push/potted_meat_can/potted_meat_can.urdf",
            "rl_env_assets/nonprehensile_manipulation/object_push/apple/apple.urdf",
            "rl_env_assets/nonprehensile_manipulation/object_push/tomato_soup_can/tomato_soup_can.urdf"
        ]
        
        # Randomly select an object path
        self._object_name = self.np_random.choice(object_paths)
        self.object_path = add_assets_path(self._object_name)
        
        # Remove the existing object if it exists
        if hasattr(self, 'obj_id') and self.obj_id is not None:
            self._pb.removeBody(self.obj_id)
        
        # Load the new randomly selected object
        ...
        # Reset the position of the object with random orientation if enabled
        ...
        # Physical parameters can be randomized here
    ...
    def load_trajectory(self):
        ...
        for i in range(int(self.traj_n_points)):
            ...
            self._pb.changeVisualShape(traj_point_id, -1, rgbaColor=[0, 1, 0, 0]) # modify trajectory here
    ...
    def update_trajectory(self):
        ...
        for i in range(int(self.traj_n_points)):
            ...
            self._pb.changeVisualShape(self.traj_ids[i], -1, rgbaColor=[0, 1, 0, 0]) # modify trajectory here
    ...
    def update_goal(self):
        ...
        if self.targ_traj_list_id >= self.traj_n_points:
            ...
            self._pb.changeVisualShape(self.goal_id, -1, rgbaColor=[0, 0, 1, 0]) # modify trajectory here
            ...
            if prev_goal_traj_list_id is not None:
                self._pb.changeVisualShape(self.traj_ids[prev_goal_traj_list_id], -1, rgbaColor=[1, 0, 0, 0]) # modify trajectory here
    ...
    def get_step_data(self):
        ...
        # get rl info
        done, type = self.termination()

        xy_info = {
            "goal_xy": [coord * 1000 for coord in self.goal_pos_worldframe[:2]],
            "object_xy": [coord * 1000 for coord in self.cur_obj_pos_worldframe[:2]],
            "type": type,
        }

        return reward, done, xy_info
    ...
    def termination(self):  
        ...
        Returns:  
            (bool, str): A tuple where the first value indicates if the episode should terminate,  
                        and the second value specifies the reason ('success' or 'timeout').  
        """  

        # Check if the object is near the goal  
        obj_goal_pos_dist = self.xyz_obj_dist_to_goal()  
        if obj_goal_pos_dist < self.termination_pos_dist:  
            # print(self._object_name)
            # Try to update the goal (if not at the end of the trajectory)  
            goal_updated = self.update_goal()  
            # If the goal cannot be updated, it means the task is successfully completed  
            if not goal_updated:  
                return True, "success"  
        ...
        # If neither condition is met, the episode should not terminate  
        return False, "none"  
