...
sys.path.insert(0, "../../") 
from tactile_gym.sb3_helpers.custom_policies import MViTacRL_SAC, MViTacRL_PPO, CL_PPO, CL_SAC
...
from tactile_gym.sb3_helpers.encoder_arsenal import *
# from tactile_gym.sb3_helpers.encoder_arsenal import *
import argparse
import torch  
import random  
...
# env name
parser.add_argument("-E", '--env_name', type=str, help='Env name.', metavar='', default='object_push-v0')
# observation mode
parser.add_argument("-O", '--observation_mode', type=str, help='Observation mode.', metavar='', default='visuotactile_and_feature')
# tactic sensor name
parser.add_argument("-S", '--tactile_sensor_name', type=str, help='Tactile sensor name.', metavar='', default=None)
# features extractor
parser.add_argument('--features_extractor_class', type=str, help='Features extractor', metavar='', default=None)
...
# algorithm name
parser.add_argument("-A", '--algo_name', type=str, help='Algorithm name.', metavar='', default='sac')
# exp name
parser.add_argument('--exp_name', type=str, help='Experiment name.', metavar='', default=None)
# timesteps
parser.add_argument('--total_timesteps', type=int, help='Total timesteps.', metavar='', default=2_000_000)
# lr
parser.add_argument('--learning_rate', type=float, help='Learning rate.', metavar='', default=3e-4)
# contrastive loss
parser.add_argument('--no_cl_loss', action='store_true', help='Enable contrastive learning loss')  
# eval freq
parser.add_argument('--eval_freq', type=int, help='Eval freq.', metavar='', default=1000)
# number of envs
parser.add_argument('--n_envs', type=int, help='Number of envs.', metavar='', default=16)
...
def save_evaluation_results(  
    file_path, features_extractor_class, cl_loss, seed, reward_mean, reward_std, length_mean, length_std  
):  
    # Create a dictionary for the new entry  
    new_entry = {  
        "features_extractor_class": features_extractor_class,  
        "cl_loss": bool(cl_loss),
        "seed": int(seed),  # In case seed is a NumPy int  
        "reward_mean": float(reward_mean),  # Convert to float  
        "reward_std": float(reward_std),  # Convert to float  
        "length_mean": float(length_mean),  # Convert to float  
        "length_std": float(length_std)  # Convert to float  
    }  
...
def train_agent(
    ...
    save_dir = os.path.join(
        "saved_models/", args.exp_name, timestr)
    ...
    if args.use_wandb:
        wandb_callback = WandbCallback(
            ...
            model_save_path=f"models/{timestr}",
            verbose=2,
            log="gradients",
        )
    ...
    if algo_name == "ppo":
        model = PPO(rl_params["policy"], env, **algo_params, verbose=1, tensorboard_log=f"runs/{timestr}")
    if algo_name == "cl_ppo":
        model = CL_PPO(rl_params["policy"], env, **algo_params, augmentations=augmentations, visualise_aug=False, verbose=1, tensorboard_log=f"runs/{timestr}")
        model.set_param(cl_loss)
    if algo_name == "cl_sac":
        model = CL_SAC(rl_params["policy"], env, **algo_params, augmentations=augmentations, visualise_aug=False, verbose=1, tensorboard_log=f"runs/{timestr}")
        model.set_param(cl_loss)
    elif algo_name == "rad_ppo":
        model = RAD_PPO(rl_params["policy"], env, **algo_params, augmentations=augmentations, visualise_aug=False, verbose=1, tensorboard_log=f"runs/{timestr}")
    elif algo_name == "sac":
        model = SAC(rl_params["policy"], env, **algo_params, verbose=1, tensorboard_log=f"runs/{timestr}")
    elif algo_name == "rad_sac":
        model = RAD_SAC(rl_params["policy"], env, **algo_params, augmentations=augmentations, visualise_aug=False, verbose=1, tensorboard_log=f"runs/{timestr}")
    elif algo_name == "mvitac_sac":
        model = MViTacRL_SAC(rl_params["policy"], env, **algo_params, augmentations=augmentations,
                             visualise_aug=False, verbose=1, tensorboard_log=f"runs/{timestr}")
...
if __name__ == "__main__":
    ...
    if args.features_extractor_class == 'VisualExtractor':
        algo_params['policy_kwargs']['features_extractor_class'] = VisualExtractor
    elif args.features_extractor_class == 'TactileExtractor':
        algo_params['policy_kwargs']['features_extractor_class'] = TactileExtractor
    elif args.features_extractor_class == 'VisualTactileExtractor':
        algo_params['policy_kwargs']['features_extractor_class'] = VisualTactileExtractor
    elif args.features_extractor_class == 'VisualTactileCMCL':
        algo_params['policy_kwargs']['features_extractor_class'] = VisualTactileCMCL
    elif args.features_extractor_class == 'VisualCMCL_atten':
        algo_params['policy_kwargs']['features_extractor_class'] = VisualCMCL_atten
    else:
        raise "no feature extractor"
